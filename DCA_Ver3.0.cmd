...
@echo off 
:menu0
    set /p s=Are you administrator? (Y/N)_
    if /i "%s%"=="Y" goto :admin
    if /i "%s%"=="N" goto :nonadmin
echo Invalid key &ping 0 -n "2">nul&cls&goto menu0
:nonadmin
@echo off
...
echo [A]: Advanced mode [X]: XP cleaning mode [D]: Launch Diskpart.exe [E]: exit
    set /p x=# Press Enter to confirm #:_
        if /i "%x%"=="A" goto :advanced2    
        if /i "%x%"=="E" goto :log2
        if /i "%x%"=="X" goto :XP2
        if /i "%x%"=="D" goto :diskpart2
echo Invalid key &ping 0 -n "2">nul&cls&goto menu4
:advanced2
color 0b
...
echo [R]: Rescan  [D]: Launch Diskpart.exe [X]: WinXP Cleaning mode [E]: Exit 
    set /p y=# Press enter to confirm: #_
        ...
        if /i "%y%"=="D" goto :diskpart2
        if /i "%y%"=="E" goto :log2
        if /i "%y%"=="X" goto :XP2
echo Invalid key &ping 0 -n "2">nul&cls&goto menu5
:diskpart2
...
echo [R]: Rescan [D]: Launch diskpart.exe [E]:exit
    set /p z=# Press Enter to confirm: #_
        ...
        if /i "%z%"=="D" goto :diskpart2
        if /i "%z%"=="E" goto :log2
echo Invalid key &ping 0 -n "2">nul&cls&goto :menu6
:admin
@echo off
